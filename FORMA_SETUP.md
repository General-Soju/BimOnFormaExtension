...
- **권장 — 브리지가 직접 서빙**: 설치 시 `standalone.html`이 `%AppData%\BimOnAI\FormaExtension\`에 배포되고,
  `BimOnMcpBridge.exe --target forma`(Claude가 자동 실행)가 이를 **`http://127.0.0.1:51737/standalone.html`** 로 제공합니다.
  → **이 URL을 그대로 사용** (별도 서버·공개 호스팅 불필요).
- **대안 — 로컬 서버**(개발 시): `python -m http.server 5173 --bind 127.0.0.1` → `http://localhost:5173/standalone.html`

> ⚠️ **공개 https URL(GitHub Pages 등)은 쓰지 마세요** — https 페이지에서 `ws://localhost` 연결을 브라우저가 차단합니다(혼합콘텐츠/PNA). URL은 반드시 **http**(브리지 `127.0.0.1` 또는 localhost).
...
5. **Integration → Embedded views**:
   ...
   - URL = **`http://127.0.0.1:51737/standalone.html`** (브리지 제공 — A 참고)
...
- 브리지 URL(`127.0.0.1:51737`)을 쓰면 별도 서버가 필요 없습니다. (대안인 로컬 서버 URL을 쓸 때만 그 서버를 켜둬야 함)
